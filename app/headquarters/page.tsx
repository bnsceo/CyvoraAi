    ...
    <div className="min-h-screen bg-[#070b12] text-white">
      <main className="mx-auto max-w-7xl px-4 py-6 md:px-6 lg:px-8">
        ...
        <section className="mt-6 grid gap-4 xl:grid-cols-[1.15fr_.85fr]">
          ...
          <article className="cyvora-glass rounded-3xl p-5 md:p-6"><p className="text-[10px] uppercase tracking-[0.22em] text-emerald-200/70">Runtime health</p><h2 className="mt-2 text-xl font-semibold text-white">Operating posture</h2><div className="mt-4 space-y-3"><HealthRow label="Executive AI" value={data?.executive_ai.status || 'unknown'} /><HealthRow label="Companies" value={String(data?.totals.companies || 0)} /><HealthRow label="Active agents" value={String(data?.totals.agents || 0)} /><HealthRow label="Queued tasks" value={String(data?.totals.tasks || 0)} /><HealthRow label="Pending approvals" value={String(data?.totals.approvals || 0)} /></div><div className="mt-5 flex flex-wrap gap-2"><Link href="/war-room" className="cyvora-chip px-4 py-2 text-xs text-amber-100">Open War Room</Link><Link href="/history" className="cyvora-chip px-4 py-2 text-xs text-slate-300">Open History</Link></div></article>
        </section>
      </main>
